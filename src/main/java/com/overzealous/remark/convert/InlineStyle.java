...
public class InlineStyle extends AbstractNodeHandler {
	...
	private static final Pattern BOLD_PATTERN = Pattern.compile("font-weight:\\s*(bold|700)", Pattern.CASE_INSENSITIVE);
	...
	private void start(Rules style, String leadingSpaces, DocumentConverter converter) {
		...
		if(style.italics) {
			if(italicDepth == 0) {
				converter.output.write("<span class='"+ITALICS_CLASS+"'>");
			}
		}
		if(style.bold) {
			if(boldDepth == 0) {
				converter.output.write("<span class='"+BOLD_CLASS+"'>");
			}
		}
