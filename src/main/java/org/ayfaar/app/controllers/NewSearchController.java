...
public class NewSearchController {
    ...
    public SearchResultPage search(String query, Integer pageNumber, SearchFilter filter) {
        ...
        if (term != null) {
            ...
            List<Item> foundItems = searchInDb(allPossibleSearchQueries, pageNumber, filter);
        } else {
            // 4. Поиск фразы (не термин)
            List<Item> foundItems = searchInDb(query, pageNumber, filter);
        }
    ...
    private List<Item> searchInDb(String query, Integer page, SearchFilter filter) {
        throw new NotImplementedException();
    }

    private List<Item> searchInDb(List<String> words, Integer page, SearchFilter filter) {
        throw new NotImplementedException();
    }
